...
export default function MessagesPage() {
  ...
  const fetchMessages = async () => {
    try {
      ...
      if (!response.ok) throw new Error("메시지 목록을 불러올 수 없습니다")
  ...
  const fetchCourses = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        setCourses(data)
        // 첫 번째 강의의 교원을 기본 수신자로 설정
        if (data.length > 0 && !formData.receiverId) {
          setFormData((prev) => ({
            ...prev,
            courseId: data[0].id,
            receiverId: data[0].instructor.id,
          }))
        }
      }
    } catch (err) {
  ...
  const resetForm = () => {
    setFormData({
      ...
      receiverId: courses.length > 0 ? courses[0].instructor.id : "",
      subject: "",
      content: "",
    })
    setShowForm(false)
  ...
  const handleCourseChange = (courseId: string) => {
    ...
    if (course) {
      setFormData({
        ...formData,
        courseId,
        receiverId: course.instructor.id,
      })
    }
  }
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-gray-600">담당교원과 메시지를 주고받을 수 있습니다</p>
          </div>
          <button
        ...
        {showForm && (
          <div className="mb-6 rounded-lg bg-white p-6 shadow">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700">
                  수신자 (담당교원) <span className="text-red-500">*</span>
                </label>
                <select
                  required
                ...
                >
                  ...
                  {courses.map((course) => (
                    <option key={course.instructor.id} value={course.instructor.id}>
                      {course.instructor.name} ({course.title})
                    </option>
                  ))}
                </select>
              </div>
